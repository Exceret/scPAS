...
#' @title Linear Model with L0 Regularization
#' @description
#' Fits linear regression models with L0 regularization for feature selection.
...
#' @param ill Whether to use log-likelihood for model selection
...
LmL0 = function(
    ...
    out$Beta = Matrix::Matrix(
        out$Beta[, 1:nlambdai, drop = FALSE] * sdy,
        sparse = TRUE
    )
    ...
    if (nfolds == 1 & is.null(foldid)) {
        ...
        for (i in 1:nfolds) {
            ...
            outi[[i]]$Beta[is.na(outi[[i]]$Beta)] = outi[[i]]$BetaSTD[is.na(
                outi[[i]]$Beta
            )]

            if (ill) {
                delta2i = outi[[i]]$RSS[1:outi[[i]]$nlambda] / N0i[i]
                cvRSS[i, 1:outi[[i]]$nlambda] = outi[[i]]$RSSp[
                    1:outi[[i]]$nlambda
                ] /
                    delta2i +
                    Nf[i] * log(delta2i) ## for ith fold
            } else {
                cvRSS[i, 1:outi[[i]]$nlambda] = outi[[i]]$RSSp[
                    1:outi[[i]]$nlambda
                ] ## for ith fold
        ...
        if (icutB) {
            ...
            repeat {
                ...
                cut0 = sort(unique(c(0, abs(BetaSTD))), decreasing = FALSE)
                cuti = NULL
                for (i in 1:ncutB) {
                ...
                cvRSS = matrix(NA, nrow = nfolds, ncol = length(cut0))
                i = 1
                for (i in 1:nfolds) {
                    temid = foldid == i
                    Betaj = Betai[, i]
            ...
            if (aPen) {
                ...
                cuti = which.min(temi[sum(wbeta == 0):length(temi)]) +
                    sum(wbeta == 0) -
                    1
            }
