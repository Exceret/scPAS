...
CoxL0 <- function(
  ...
  if (nfolds == 1 && is.null(foldid)) {
    ...
    temi <- Matrix::colSums(abs(cvPL) == Inf, na.rm = TRUE)
    if (any(temi > 0)) {
      nlambdai <- max(min(which(temi > 0)) - 1, 1)
    }

    cvraw <- cvPL / weighti
    nfoldi <- Matrix::colSums(!is.na(cvraw)) #rm(cvPL) #
    ...
    if (icutB) {
      ...
      repeat {
        ...
        cvPL <- matrix(NA, nrow = nfolds, ncol = length(cut0))
        i <- 1
        for (i in 1:nfolds) {
          Betaj <- Betai[, i]
          BetaSTDj <- BetaSTDi[, i]
        ...
        if (ifast) {
          if (sum(!is.na(cv.max)) > 1) {
            if (
              !is.na(cv.max[pmin(il1 + 1, nlambdai)]) &&
                !is.na(cv.max[pmax(il1 - 1, 1)])
            ) {
              break
      ...
      if (aPen) {
        ...
        cuti <- which.max(temi[sum(wbeta == 0):length(temi)]) +
          sum(wbeta == 0) -
          1
      }
